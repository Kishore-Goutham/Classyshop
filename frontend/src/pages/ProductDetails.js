...
function ProductDetails() {
  let { SetcartProducts,fetchMyList } = useContext(dataContext);
  ...
  useEffect(() => {
    ...
    async function fetchRelated() {
      let related = await fetchDataFromApi(
        `/api/product/getProductsByCatId/${productData.catId}`,
      );
      if (related.success) {
  ...
  async function handleCart() {
    if (
      productData.catName === "Fashion" ||
      productData.catName === "Footwear"
    ) {
      if (!size) {
    ...
    try {
      let data = await postData("/api/cart/add", {
        productId: id,
        quantity: qty,
        size: size,
        ram: ram,
      ...
      <div className="md:flex gap-10 container mx-auto">
        ...
        <div className=" mt-4 md:w-[60%]">
          ...
          <span className="flex items-center gap-2 mt-4">
            <span> Brands :</span> <span>{productData.brand}</span>{" "}
            <Rating
              name="size-small"
          ...
          <div className="flex mt-4 items-center gap-2">
            ...
            <h1 className="text-[#ff5252] text-lg font-medium">
              ${productData.price}
            </h1>
          </div>
          <div className="flex items-center gap-2 mt-4 mb-2">
            {productData.countInStock === 0 ? (
              ...
              <span className="bg-green-500 text-white rounded-full p-2 text-sm">
                In stock
              </span>
            )}
            {/* <p className='text-xl font-medium'>{ productData.countInStock}</p>  */}
          </div>
          <p className="text-justify mt-4 font-light">
          ...
          <div className="flex items-center mt-5 gap-5">
            ...
            <div>
              <Button
                className="flex gap-2 !bg-[#ff5252] !text-white md:!p-2"
                onClick={handleCart}
              >

...
function Productitem({ data }) {
  ...
  async function addMylist(id) {
    console.log(id)
    try {
      let data = await postData(`/api/myList/addToMyList`,{productId:id});
      if (data.success) {
  ...
  return (
    <>
      <div className="w-[100%] rounded-md overflow-hidden border-2 border-gray-200">
        <div className="h-[180px] md:h-[250px] bg-white relative productitem overflow-hidden">
          ...
          <div className="absolute top-[-500px] right-2 hover:top-2 actions">
            <Button onClick={()=>addMylist(data?._id)} className="!text-black !min-w-7 !rounded-full !bg-white hover:!bg-[#ff5252] hover:!text-white hover:cursor-pointer">
              <FaRegHeart />
            </Button>
          </div>

          <div className="absolute top-2 left-3 w-max p-1 rounded-full bg-red-400 text-white">
            {data?.discount}%
          </div>
        </div>
        ...
        <div className="p-2 bg-[#f1f1f1]">
          <Link className="link">
            <p className="mt-2 text-sm">{data?.brand}</p>
          </Link>
          ...
          >
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">Brand: {data?.brand}</p>
                <Link to={`/productdetails/${data?._id}`} className="link">
                  View Details
                </Link>
              </div>
            </div>
            ...
            {/* Size Dropdown */}
            {(data.catName === "Fashion" || data.catName === "Footwear") && (
              <div className="mt-4">
            ...
            {/* Price */}
            <div className="mt-4">
              <p className="text-xl font-bold text-[#ff5252]">₹{data?.price}</p>
            </div>

...
const columns = [
  ...
  { id: "thirdcat", label:"Third level category", minWidth: 100 },
  { id: "action", label: "Action", minWidth: 100 },
];

function Thirdlevelcat() {
                         ...
                         <div className="flex gap-3">
                               {data.children.map((data,index)=>{
                                return (data.children.map((item,index)=>{
                                   return <span className="bg-blue-500 p-2 rounded-full text-white">{item.name}</span>
                                }))
                               })}
                         </div>
                      </TableCell>

...
function Addthirdlevelcat() {
  ...
  function selectCatName(name) {
   ...
   console.log(formfields,productCat)
  async function handleSubmit(e) {
    e.preventDefault();
    Setisloading(true);
    try {
      let data = await postData("/api/category/createCategory", {
        ...formfields,
        parentId: productCat,
      });
      if (data.success) {
    ...
    <section className="bg-gray-100 py-8 px-14">
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 mb-5">
          <div>
            <h2 className="font-medium mb-2">Sub Category Name</h2>
            <Select
              className="w-[25%] h-11"
              size="small"
            ...
            >
              {catData.map((item, index) => {
                return (item.children.map((data, index) => {
                  return (
                    <MenuItem
                      value={data._id}
                      onClick={() => selectCatName(data.name)}
                    >
                      {data.name}
                    </MenuItem>
                  );
          ...
          <div>
            <h1 className="text-lg mb-1">Third-Category Name</h1>
            <input
              type="text"
              id="name"
...
export default Addthirdlevelcat;
